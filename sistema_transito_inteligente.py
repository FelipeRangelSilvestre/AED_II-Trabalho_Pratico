...
# -*- coding: utf-8 -*-
"""
Sistema de Trânsito Inteligente - VERSÃO PROFISSIONAL (COM PERSISTÊNCIA)
Malha 10x10 com 100+ interseções, vias de mão única, becos e visual Google Maps.
Inclui funcionalidade de Salvar/Carregar em JSON.
"""
...
import time
import random
import json  # Biblioteca para persistência de dados
...
class ArvoreAVL:
    ...
    # --- NOVO: Método para limpar a árvore ao carregar novo arquivo ---
    def limpar(self):
        self.root = None
        self.total_eventos = 0
        self.eventos.clear()
...
class GrafoPonderado:
    ...
    # --- NOVO: Método para limpar o grafo ao carregar novo arquivo ---
    def limpar(self):
        self.vertices.clear()
        self.arestas.clear()
...
class SistemaTransitoMelhorado:
    ...
    # --- NOVO: MÉTODOS DE PERSISTÊNCIA (SALVAR/CARREGAR) ---
    def salvar_dados(self, nome_arquivo):
        ...
        # Salvar Vias (Arestas + Metadados)
        vias_processadas = set()
        for origem in self.grafo.vertices:
            if origem in self.grafo.arestas:
                for destino, peso in self.grafo.arestas[origem].items():
                    chave = tuple(sorted([origem, destino]))
                    if chave in vias_processadas: continue
                    
                    # Recupera dados originais
                    info = self.obter_info_via(origem, destino)
                    if info:
                        vias_processadas.add(chave)
                        dados["vias"].append({
                            "origem": origem,
                            "destino": destino,
                            "distancia": info['distancia_km'],
                            "velocidade_padrao": self.velocidades.get(f"{origem}-{destino}", self.VELOCIDADE_PADRAO)
                        })
        ...
        for evento in self.avl.listar_todos():
            dados["eventos"].append({
                ...
                "aumento_tempo": evento.aumento_tempo
            })
    ...
    def carregar_dados(self, nome_arquivo):
        try:
            ...
            # 2. Restaurar Vértices
            for v in dados["vertices"]:
                self.grafo.adicionar_vertice(v)

            # 3. Restaurar Vias
            for via in dados["vias"]:
                self.adicionar_via(
                    via["origem"], 
                    via["destino"], 
                    via["distancia"], 
                    via.get("velocidade_padrao", 60.0)
                )

            # 4. Restaurar Eventos
            maior_id = 0
            for ev in dados["eventos"]:
                origem, destino = ev["localizacao"].split('-')
                # Registra o evento novamente para recalcular pesos
                self.registrar_evento(ev["tipo"], origem, destino)
                
                # Ajusta ID e Timestamp para serem fiéis ao original
                # (O registrar_evento cria um novo ID, aqui forçamos o antigo)
                novo_evento = self.avl.buscar(self.proximo_id_evento - 1)
                if novo_evento:
                    # Remove do dicionário com ID novo e põe com ID antigo
                    del self.avl.eventos[novo_evento.id]
                    novo_evento.id = ev["id"]
                    novo_evento.timestamp = ev["timestamp"]
                    self.avl.eventos[ev["id"]] = novo_evento
                
                if ev["id"] > maior_id:
                    maior_id = ev["id"]
            
            self.proximo_id_evento = maior_id + 1
...
class InterfaceTransitoMelhorada:
    ...
    def __init__(self, root):
        ...
        self.root.after(5000, self.simular_eventos_aleatorios)  # Eventos nas rotas calculadas
    ...
    def simular_eventos_aleatorios(self):
        # Só cria eventos se houver rotas calculadas
        if not self.caminhos_alternativos:
            self.root.after(5000, self.simular_eventos_aleatorios)
            return
        
        tipos_evento = list(self.sistema.VELOCIDADES_EVENTO.keys())
        
        # Coleta todas as vias das rotas calculadas
        vias_das_rotas = []
        for caminho in self.caminhos_alternativos:
            for i in range(len(caminho) - 1):
                origem, destino = caminho[i], caminho[i + 1]
                # Verifica se a via existe
                if self.sistema.obter_info_via(origem, destino):
                    vias_das_rotas.append((origem, destino))
        
        # Remove duplicatas
        vias_das_rotas = list(set(vias_das_rotas))
        
        if vias_das_rotas:
            origem, destino = random.choice(vias_das_rotas)
            ...
            if evento_na_via:
                # 30% de chance de remover evento existente
                if chance < 0.3:
                    self.sistema.remover_evento(evento_na_via.id)
                    self.atualizar_interface()
            else:
                # 25% de chance de criar novo evento (reduzido para não poluir)
                if chance > 0.75:
    ...
    def criar_menu(self):
        ...
        # --- NOVO: ITENS DE MENU PARA SALVAR/CARREGAR ---
        menu_arquivo.add_command(label="💾 Salvar Estado", command=self.salvar_estado)
        menu_arquivo.add_command(label="📂 Carregar Estado", command=self.carregar_estado)
    ...
    def desenhar_grafo(self):
        ...
        for origem, destino, _ in self.grafo.obter_todas_arestas():
            ...
            # Cor e largura das ruas
            cor = "#424242"  # CINZA ESCURO para vias normais
            largura = 2
            
            is_on_main_path = self._is_segment_on_path(origem, destino, self.caminho_atual)
            
            # Verifica se há evento ATIVO nesta via
            eventos_ativos = self.sistema.avl.listar_todos()
            has_event = False
            for ev in eventos_ativos:
                loc = ev.localizacao
                if loc == f"{origem}-{destino}" or loc == f"{destino}-{origem}":
                    has_event = True
                    break
            ...
            if is_on_main_path:
                ...
                largura = 4
                 ...
                 cor = "#E53935"  # Vermelho - APENAS para eventos ativos
                 largura = 3
        ...
        # Desenhar vértices (interseções) - CINZA CLARO
        for vertice, (x, y) in self.vertices_pos.items():
            cor = "#E0E0E0"  # CINZA CLARO para vértices normais
            outline_cor = "#9E9E9E"  # CINZA MÉDIO para borda
            raio = 8
            mostrar_label = False
            label_cor = "#424242"
            ...
            if self.caminho_atual:
                if vertice == self.caminho_atual[0]:
                    ...
                    self.canvas.create_text(x, y+raio+15, text=vertice, fill="#34A853", 
                                          font=("Arial", 8, "bold"), anchor=tk.N)
                    continue
                    ...
                    self.canvas.create_text(x, y-raio-15, text="📍 DESTINO", fill="#EA4335", 
                                          font=("Arial", 9, "bold"), anchor=tk.S)
                    self.canvas.create_text(x, y+raio+15, text=vertice, fill="#EA4335", 
                                          font=("Arial", 8, "bold"), anchor=tk.N)
                    continue
            ...
            if vertice.startswith('BECO'):
                ...
                self.canvas.create_text(x, y+raio+10, text="🚫", fill="#F57C00", 
                                      font=("Arial", 10))
                self.canvas.create_text(x, y+raio+25, text=vertice, fill="#F57C00", 
                                      font=("Arial", 7, "bold"), anchor=tk.N)
                continue
                ...
                self.canvas.create_text(x, y+raio+15, text=vertice, fill="#E53935", 
                                      font=("Arial", 7, "bold"), anchor=tk.N)
                continue
            
            self.canvas.create_oval(x-raio, y-raio, x+raio, y+raio, fill=cor, outline=outline_cor, width=1)
            
            # Adiciona label do nome do vértice
            if mostrar_label or (not self.caminho_atual):
                # Mostra labels em vértices da rota ou quando não há rota calculada
                self.canvas.create_text(x, y+raio+12, text=vertice, fill=label_cor, 
                                      font=("Arial", 7), anchor=tk.N)
    ...
    def calcular_layout_grid(self):
        ...
        margin = 60  # Aumentado para evitar que nós toquem as bordas
    ...
    # --- NOVO: FUNÇÕES DE GUI PARA SALVAR/CARREGAR ---
    def salvar_estado(self):
        ...
        if filename:
            sucesso, msg = self.sistema.salvar_dados(filename)
            if sucesso:
    ...
    def carregar_estado(self):
        ...
        if filename:
            confirmar = messagebox.askyesno("Confirmar", "Isso irá substituir a malha atual. Continuar?")
            if confirmar:
                sucesso, msg = self.sistema.carregar_dados(filename)
                if sucesso:
                    messagebox.showinfo("Sucesso", msg)
                    self.caminho_atual = []
                    self.caminhos_alternativos = []
                    self.text_resultado.config(state=tk.NORMAL)
                    self.text_resultado.delete("1.0", tk.END)
                    self.text_resultado.insert("1.0", "📂 Dados carregados!\nUse os controles para calcular novas rotas.")
                    self.text_resultado.config(state=tk.DISABLED)
                    self.atualizar_interface()
                else:
                    messagebox.showerror("Erro", msg)
    ...
    def criar_dados_exemplo(self):
        ...
        # Expressas diagonais (usando adicionar_via para garantir que sejam tratadas corretamente)
        # Diagonal superior esquerda para centro
        self.sistema.adicionar_via('R1C1', 'R2C2', 1.7, 70.0)
        self.sistema.adicionar_via('R2C2', 'R3C3', 1.7, 70.0)
        self.sistema.adicionar_via('R3C3', 'R4C4', 1.7, 70.0)
        self.sistema.adicionar_via('R4C4', 'R5C5', 1.7, 70.0)
        
        # Diagonal superior direita para centro
        self.sistema.adicionar_via('R1C10', 'R2C9', 1.7, 70.0)
        self.sistema.adicionar_via('R2C9', 'R3C8', 1.7, 70.0)
        self.sistema.adicionar_via('R3C8', 'R4C7', 1.7, 70.0)
        self.sistema.adicionar_via('R4C7', 'R5C6', 1.7, 70.0)
        self.sistema.adicionar_via('R5C6', 'R5C5', 0.8, 70.0)
        
        # Diagonal inferior esquerda para centro
        self.sistema.adicionar_via('R10C1', 'R9C2', 1.7, 70.0)
        self.sistema.adicionar_via('R9C2', 'R8C3', 1.7, 70.0)
        self.sistema.adicionar_via('R8C3', 'R7C4', 1.7, 70.0)
        self.sistema.adicionar_via('R7C4', 'R6C5', 1.7, 70.0)
        self.sistema.adicionar_via('R6C5', 'R5C5', 1.2, 70.0)
        
        # Diagonal inferior direita para centro
        self.sistema.adicionar_via('R10C10', 'R9C9', 1.7, 70.0)
        self.sistema.adicionar_via('R9C9', 'R8C8', 1.7, 70.0)
        self.sistema.adicionar_via('R8C8', 'R7C7', 1.7, 70.0)
        self.sistema.adicionar_via('R7C7', 'R6C6', 1.7, 70.0)
        self.sistema.adicionar_via('R6C6', 'R5C5', 1.7, 70.0)
...
• PERSISTÊNCIA: Salve e Carregue seus cenários!

🎯 COMO USAR:
1. Menu "Arquivo": Salve ou Carregue o estado
2. Selecione origem e destino
3. "Calcular Rota" = melhor opção
4. "Mostrar 3 Alternativas" = todas as rotas
